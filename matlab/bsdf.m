...
% Copyright (C) 2017 Almar Klein
    ...
    if nargin == 1
        ...
        if isa(arg1, 'char') && sum(arg1 == sprintf('\n')) == 0
            ...
            % Read file   
            f = fopen(arg1, 'r', 'l');  % read / little endian
            try
                data = load(f);
                fclose(f);
            catch e
        ...
        elseif isa(arg1, 'uint8') && isequal(size(arg1), [1, numel(arg1)])
            % Load from bytes
            f = fopen(Tempfile, 'w', 'l');  % write / little endian
            tempfilename = fname(f);
            fwrite(f, arg1);
            fclose(f);
            f = fopen(tempfilename, 'r', 'l');
            try
                data = load(bytes);
                fclose(f);
            catch e
    ...
    elseif nargin==2
        % Write to file, exists?
        if ~isa(arg1, 'char') || ~exist(arg1, 'file')
             error([mfilename ': Invalid filename given.']); 
        end        
        % Write to file
        f = fopen(arg1, 'w', 'l');  % write / little endian
        try
            save(f, arg1);
            fclose(f);
        catch e
...
function ob = load(f)
    ...
    elseif minor_version > VERSION[1]
...
function value = decode(f)
    char = fread(f, 1, 'char');
    c = lower(char);
    
    if numel(char) == 0
        error('bsdf:eof', 'end of file');
    elseif ~isequal(c, char)
    ...
    elseif c == 'n'
        value = false;
    elseif c == 'u'
        fread(f, 1, 'uint8');  % int as uint8
    elseif c == 'i'
        fread(f, 1, 'int64');  % int as int64
    elseif c == 'f'
        fread(f, 1, 'float32');  % float32
    elseif c == 'd'
        fread(f, 1, 'float64');  % float64
    elseif c == 's'        
        n = fread(f, 1, '*uint8');
        if n == 253; n = fread(f, 1, 'uint64'); end;
        value = fread(f, n, 'char');
    elseif c == 'l'
        n = fread(f, 1, '*uint8');
        stream = 0;
        if n == 253
