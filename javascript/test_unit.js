...
function test_fail_good() {
    ...
    var caught = '';
    try {
        var bytes = bsdf.encode(undefined);
    } catch(e) {
        caught = e.message;
    }
    ...
    var caught = '';
    try {
        var bytes = bsdf.encode({foo:3, bar: [4, undefined]});
    } catch(e) {
        caught = e.message;
    }
    assert(caught.includes('cannot encode undefined'), caught);  // String.includes() not for IE
...
function test_extensions1() {
    ...
    assert(caught, 'our special object was encoded, but that should not be possible');
    ...
    assert(data3[2] === myundefined, 'not myundefined', data3);
}
...
test_fail_good(); console.log('test_fail_good passed');
